...
System *SystemSemanticAction(char *name, System *system) {
    _logSyntacticAnalyzerAction(__FUNCTION__);
    if (system == NULL) { system = EmptySystemSemanticAction(); }
    system->name = name;
    return system;
}
...
Units *AddMassUnitToUnitsSemanticAction(Units *units, const MassUnit massUnit) {
    _logSyntacticAnalyzerAction(__FUNCTION__);
    if (units == NULL) { units = EmptyUnitsSemanticAction(); }
...
Body *BodySemanticAction(char *name, Body *body) {
    _logSyntacticAnalyzerAction(__FUNCTION__);
    if (body == NULL) { body = EmptyBodyItemsSemanticAction(); }
    body->name = name;
    return body;
}
...
Body *MergeBodySemanticAction(Body *left, Body *right) {
    ...
    if (left == NULL) { return right; }
    if (right == NULL) { return left; }
    if (right->shape != BODY_SHAPE_DEFAULT) { left->shape = right->shape; }
    if (right->mass != NULL) {
